...
export async function addNewJobToRelevantRecommendations(job: Job) {
  ...
  for (const profile of profiles) {
    ...
    if (match.score >= 80) await createNotification({ userId: profile.userId, type: "high_match", title: `${match.score}% relevance match`, body: `${job.title} at ${job.company} is a new high-relevance role based on your confirmed profile.`, href: `/jobs/${job.id}`, fingerprint: `high-match:${job.id}` });
    added += 1;
  }
